...
UniformBufferObject *Instance::state(const glm::mat4& view, const glm::mat4& proj, const glm::mat4& view_1proj_1, float aspectRatio) {
    ...
    if (renderAsIcon) {
        auto ndc = proj * view * glm::vec4(position.x, position.y, position.z, 1.0);

        _state.model = view_1proj_1 * translate(glm::vec3(ndc.x / ndc.w, ndc.y / ndc.w, 0.1f)) * scale(glm::vec3(0.1f / aspectRatio, 0.1f, 0.1f));
    } else {
